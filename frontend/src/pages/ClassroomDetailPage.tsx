    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{classroom.occupancy} / 45</p>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{classroom.temperature}°C</p>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <p className="text-2xl font-bold text-foreground">{classroom.humidity}%</p>
        ...
        <div className="rounded-2xl border border-border bg-card p-5">
          ...
          <div className="flex items-baseline gap-2">
            <p className="text-2xl font-bold text-foreground">{classroom.riskScore}</p>
        ...
        <div className="rounded-2xl border border-border bg-card p-5 space-y-4">
          ...
          <div className="space-y-3 divide-y divide-border">
            <div className="flex justify-between items-center pt-2">
              ...
              <span className="text-sm font-bold text-foreground">{classroom.currentPower} kW</span>
            </div>

            <div className="flex justify-between items-center pt-2">
              <span className="text-xs text-muted-foreground">Expected Power</span>
              <span className="text-sm font-bold text-foreground">{classroom.expectedPower} kW</span>
            </div>
        ...
        <div className="h-[250px] w-full">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={roomTrendData} margin={{ top: 5, right: 10, left: -10, bottom: 5 }}>
              ...
              />
              <Line type="monotone" dataKey="actual" name="Actual (kW)" stroke="hsl(142, 55%, 42%)" strokeWidth={2.5} dot={false} />
              <Line type="monotone" dataKey="predicted" name="Expected (kW)" stroke="hsl(80, 75%, 50%)" strokeWidth={2} strokeDasharray="5 3" dot={false} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-2">
        ...
        <div className="rounded-2xl border border-border bg-card p-5 space-y-3">
          ...
          <div className="rounded-xl border border-amber-200/60 bg-amber-50/50 dark:bg-amber-500/10 dark:border-amber-500/20 p-4 text-xs text-foreground">
            {roomRecs.length > 0 ? (
              roomRecs[0].message
            ) : (
            ...
            <div className="space-y-2">
              {roomAlerts.map((alert) => (
                <div key={alert.id} className="rounded-xl border border-border bg-muted/30 p-3 text-xs">
                  ...
                  <p className="text-[10px] text-muted-foreground mt-1">Severity: {alert.severity.toUpperCase()}</p>
                </div>
              ))}
            </div>
          ) : (
